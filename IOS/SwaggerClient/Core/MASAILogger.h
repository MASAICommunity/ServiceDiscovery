...
#ifndef MASAIDebugLogResponse
#define MASAIDebugLogResponse(response, responseObject,request, error) [[MASAILogger sharedLogger] logResponse:response responseObject:responseObject request:request error:error];
#endif

/**
 * Log debug message macro
 */
#ifndef MASAIDebugLog
#define MASAIDebugLog(format, ...) [[MASAILogger sharedLogger] debugLog:[NSString stringWithFormat:@"%s", __PRETTY_FUNCTION__] message: format, ##__VA_ARGS__];
#endif

@interface MASAILogger : NSObject

+(instancetype)sharedLogger;

/**
 * Enabled switch, default NO - default set by MASAIConfiguration debug property
 */
